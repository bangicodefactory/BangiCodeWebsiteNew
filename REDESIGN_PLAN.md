...
- All v4 sections present and ordered correctly: hero, thesis-line stats, trusted-by row, services grid (with hover state on each card), featured RentCar case with 4 metric tiles and stack+industry pills, more-work peek (Coinluminaire / Classkom / Aqarchamal) with pills, testimonial, 4-step "what happens next", founder contact card, 4-column footer, legal+social bottom row.
  - **The studio-status panel was removed on 2026-08-14** and is deliberately absent — do not report it as a missing section. Its fields (current sprint, next availability, local time) went stale because nothing kept them current. See the reversal note in [ADR 0001](docs/adr/0001-adopt-claude-design-system-tokens.md).
- ~~Studio-status `● online` indicator uses `bg-tertiary-fixed-dim` (`#ffb4a8`)~~ — moot; the panel and its dot are gone. The colour decision itself was already superseded by ADR 0001, which replaced the raw hex with the `spark` token.
